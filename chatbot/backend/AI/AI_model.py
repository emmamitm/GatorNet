...
if 'AI_DATA_DIR' in os.environ:
    ...
    for _ in range(5):  # Try up to 5 levels up
        ...
        print(f"WARNING: Could not find scrapedData directory. Using {HOME_DIR} as HOME_DIR")
...
LIBRARY_ALIASES = {
    ...
    "health science center library": ["health library", "hscl", "health science library"],
    "architecture & fine arts library": ["afa", "architecture library", "fine arts library"],
...
BUILDING_ALIASES = {
    ...
    "ben hill griffin stadium": ["the swamp", "stadium", "football stadium", "ben hill"],
    ...
    try:
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                nearby_locations = row.get('nearby_locations_str', '').split(',') if row.get('nearby_locations_str') else []
                ...
                if 'Hours' in row and row['Hours']:
                    try:
                        ...
                        day_patterns = [
                            ...
                            (r"([\d:]+\s*[ap]m\s*-\s*[\d:]+\s*[ap]m)\s*\((Fri)\)", ["Friday"]),
                            (r"([\d:]+\s*[ap]m\s*-\s*[\d:]+\s*[ap]m)\s*\((Sat)\)", ["Saturday"]),
                            (r"([\d:]+\s*[ap]m\s*-\s*[\d:]+\s*[ap]m)\s*\((Sun)\)", ["Sunday"])
                        ]
                        ...
                        for pattern, days in day_patterns:
                            ...
                            if matches:
                                for match in matches:
                                    hour_str = match[0] if isinstance(match, tuple) else match
                                    for day in days:
...
class ConversationState:
    ...
    def add_message(self, speaker: str, message: str):
        ...
        if speaker == "Assistant":
            ...
            self.awaiting_followup = message.rstrip().endswith("?") or "would you like" in message.lower()
        ...
        # If it's the user's message, update context
            ...
            if session_duration > 20:
                ...
                logger.info(f"Long conversation detected: {session_duration:.1f} minutes")
    ...
    def is_followup_question(self, query):
        ...
        for entity_type in ['libraries', 'buildings', 'dorms', 'courses', 'majors', 'clubs']:
    ...
    def get_dominant_interests(self):
        ...
        dominant_intents = [intent for intent, count in self.intent_counts.items() 
                           if count == max_count]
        ...
        if isinstance(previous_context, dict):
            ...
            if entity_type and entity_name:
                ...
                for mentioned in self.mentioned_entities[entity_type]:
                    if (mentioned.lower() in query_lower and 
                        mentioned.lower() != entity_name.lower() and
                        len(mentioned) > 3):  # Avoid short name false positives
                ...
                context_switchers = [
                    "instead", "different", "another", "not that", "other than"
                ]
...
class AcademicInfoRetrieval:
    def __init__(self, embedding_model: SentenceTransformer):
        ...
        logger.info(f"Initialized academic info retrieval with {len(self.programs_data)} programs, {len(self.majors_data)} majors, and {len(self.courses_data)} courses.")
    ...
    def get_info(self, query: str) -> dict:
        ...
        if query_lower in self.search_terms:
            ...
            query_simple = query_lower.replace("major", "").replace("program", "").strip()
        ...
        if matched_dept:
            ...
            for entry in self.majors_data:
                if entry.get("Department", "").lower() == matched_dept:
                    result["description"] = entry.get("Description", result["description"])
                    break
        ...
        if not result["programs"]:
            ...
            for program in self.programs_data:
                ...
                if query_lower in name:
                    ...
                    # Look for a description
                    for entry in self.majors_data:
                        if entry.get("Department", "").lower() == result["department"].lower():
                            result["description"] = entry.get("Description", result["description"])
                            break
        ...
        if not result["programs"]:
            ...
            matches = difflib.get_close_matches(query_lower, department_names, n=1, cutoff=0.7)
...
class CampusClubsRetrieval:
    def __init__(self, embedding_model: SentenceTransformer):
        ...
        self.query_cache = LRUCache(capacity=100)
        logger.info(f"Initialized campus clubs retrieval with {len(self.club_data)} entries.")
    ...
    def get_club_info(self, query: str) -> str:
        ...
        for entry in self.club_data:
            ...
            if (query_lower == name or 
                query_lower in name or 
                query_lower.replace(" club", "") == name or
                query_lower.replace(" organization", "") == name):
                    
                result = entry.get("Description", "No additional information available.")
                self.query_cache[cache_key] = result
                return result
        
        # Try fuzzy matching with improved threshold
        names = [entry.get("Organization Name", "") for entry in self.club_data]
        best_matches = difflib.get_close_matches(query_lower, 
                                                [name.lower() for name in names], 
                                                n=1, 
                                                cutoff=0.7)  # Higher threshold
        if best_matches:
            for entry in self.club_data:
        ...
        try:
            ...
            if best_match and best_score > 0.7:  # Higher threshold for better quality
                result = best_match.get("Description", "No additional information available.")
                self.query_cache[cache_key] = result
                return result
    ...
    def get_clubs_by_category(self, category: str) -> List[Dict]:
        ...
        category_keywords = {
            "cultural": ["cultural", "international", "multicultural", "diversity", "heritage", "ethnic"],
            "academic": ["academic", "honor", "research", "study", "education", "scholar"],
            "professional": ["professional", "career", "industry", "business", "pre-professional", "job"],
            "service": ["service", "volunteer", "community", "philanthropy", "charity", "outreach"],
            "religious": ["religious", "christian", "catholic", "jewish", "muslim", "hindu", "faith", "spiritual"],
            "sports": ["sport", "athletic", "recreation", "fitness", "outdoor", "team", "physical", "game"],
            "greek": ["fraternity", "sorority", "greek", "panhellenic", "ifc", "chapter"]
        }
        ...
        if category_lower in category_keywords:
            ...
            for cat, words in category_keywords.items():
                ...
                # If still no match, try fuzzy matching
                best_match = difflib.get_close_matches(category_lower, category_keywords.keys(), n=1, cutoff=0.6)
                if best_match:
...
class QueryAnalyzer:
    def __init__(self):
        ...
        self.intent_patterns = {
            "library_hours": [
                r'(?:library|lib).*hours', r'hours.*(?:library|lib)', 
                r'when.*(?:library|lib).*open', r'(?:library|lib).*open.*?when', 
                r'(?:library|lib).*close', r'is (?:library|lib) open', 
                r'(?:library|lib).*schedule'
            ],
            "building_location": [
                r'where is', r'location of', r'located', r'address', r'find',
                r'get to', r'directions to', r'how.*get to', r'where.*find', r'map'
            ],
            "dorm_info": [
                r'dorm', r'housing', r'residence hall', r'live', r'on-campus housing',
                r'rooms', r'housing options', r'where to live', r'accommodation', r'freshman.*housing'
            ],
            "course_info": [
                r'course', r'class', r'(?:course|class).*description', r'about.*(?:course|class)', r'syllabus',
                r'prereq', r'credit', r'professor', r'instructor', r'teach', r'[A-Z]{3}\s*\d{4}'
            ],
            "major_info": [
                r'major', r'program', r'degree', r'concentration', r'department',
                r'field of study', r'academic program', r'specialization', r'track'
            ],
            "club_info": [
                r'club', r'organization', r'group', r'society', r'association',
                r'student org', r'student group', r'extracurricular', r'activities'
            ]
        }
        
        # Intent priority order (for resolving multiple matches)
        self.intent_priority = [
            "library_hours", "building_location", "dorm_info", 
            "course_info", "major_info", "club_info"
        ]
    ...
    def analyze(self, query: str) -> dict:
        ...
        analysis = {
            "intent": "generic",
            "query": query,
            "is_location_query": any(kw in query_lower for kw in ["where", "located", "location", "address"]),
            "is_hours_query": any(kw in query_lower for kw in ["hours", "open", "close", "schedule", "when"]),
            "is_major_query": "major" in query_lower or "program" in query_lower or "degree" in query_lower,
            "is_club_query": "club" in query_lower or "organization" in query_lower,
            "is_dorm_query": any(kw in query_lower for kw in ["dorm", "housing", "residence", "live"]),
            "is_course_query": "course" in query_lower or "class" in query_lower or re.search(r'[A-Z]{3}\s*\d{4}', query),
            "is_all_query": any(kw in query_lower for kw in ["all ", "list of", "what are the", "types of"]),
            "is_personal_query": any(kw in query_lower for kw in ["my ", "i am", "for me", "my academic"])
        }
...
"""
    ...
    def generate(self, analysis, library_info=None, building_info=None, dorm_info=None, 
                major_info=None, club_info=None, academic_calendar=None):
        ...
        if intent == "library_hours" and library_info:
            try:
                return self._generate_library_hours_response(library_info, academic_calendar)
            except Exception as e:
            ...
            try:
                ...
                return self.templates["error"].format(query_subject="dormitory information")
        ...
        if self.llm:
            try:
                return self._generate_llm_response(query, analysis, library_info, building_info, 
                                             dorm_info, major_info, club_info, academic_calendar)
            except Exception as e:
    ...
    """
                    ...
                    logger.error(f"Error generating LLM response for personal query: {e}")
    ...
    def _is_uf_relevant_personal_query(self, query):
        ...
        uf_relevant_keywords = [
            "major", "program", "degree", "study", "class", "course", "career",
            "job", "college", "department", "admission", "application", "academic",
            "subject", "field", "interest", "graduate", "art", "science", "engineering", 
            "business", "medicine", "law", "education", "minor"
        ]
        ...
        has_uf_relevant = any(keyword in query_lower for keyword in uf_relevant_keywords)
        ...
        high_confidence_academic = any(term in query_lower for term in ["major", "program", "course", "degree"])
...
"""
    ...
    def _generate_major_info_response(self, major_info):
        ...
        description = major_info.get('Description', 'No detailed description available.')
    ...
    def _generate_structured_major_info(self, major_info):
        ...
        department = major_info.get('department', 'Unknown Department')
        description = major_info.get('description', 'No detailed information available.')
        ...
        if original_query:
            display_name = ' '.join(word.capitalize() for word in original_query.split())
        else:
    ...
    def _generate_llm_response(self, query, analysis, library_info=None, building_info=None, 
        ...
        prompt = self._build_llm_prompt(query, analysis, library_info, building_info, 
                                      dorm_info, major_info, club_info, academic_calendar)
        ...
        try:
            ...
            if today_str in self.calendar_data.get('library_schedule_exceptions', {}):
                result['library_schedule_exceptions'] = {
                    today_str: self.calendar_data['library_schedule_exceptions'][today_str]
                }
...
class EnhancedUFAssistant:
    ...
    def _validate_data(self):
        ...
        if not self.libraries or len(self.libraries) == 0:
            ...
            for idx, library in enumerate(self.libraries):
                ...
                if not library.get("Hours") or not isinstance(library.get("Hours"), dict):
                    validation_issues.append(f"Library '{library.get('Library Name', f'at index {idx}')}' has invalid hours format")
    ...
    def _find_building(self, query):
        ...
        return self._find_entity(query, self.campus_buildings, "Building Name", BUILDING_ALIASES)
    ...
    def process_query(self, query):
        ...
        try:
            ...
            self.conversation_state.update_intent_tracking(analysis.get("intent", "generic"))
    ...
    information - instead, offer thoughtful advice based on the user's question."""
                        ...
                        logger.info(f"Processed personal query in {processing_time:.2f} seconds")
                        
                        return response
                    except Exception as e:
                        logger.error(f"Error generating LLaMA response for personal query: {e}")
            ...
            try:
                ...
                if analysis.get("is_hours_query") or "library" in query.lower():
                    ...
                    if library_info:
                        ...
                        if not library_info.get('Hours') or not isinstance(library_info['Hours'], dict):
            ...
            try:
                ...
                if analysis.get("is_location_query") or "building" in query.lower() or "where is" in query.lower():
